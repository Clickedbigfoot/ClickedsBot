...
from .constants import DISCORD_MSG_CHAR_LIMIT, AUDIO_CACHE_PATH, PHOTO_CACHE_PATH, AMRIT_CACHE_PATH
...
class MusicBot(discord.Client):
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            else: #Following code is modified TEST
    ...
    async def on_message(self, message):
        ...
        message_content = message.content.strip() #I commented out the below so that it will handle messages besides commands /CLICKED
        ...
        if not handler and not message_content.startswith(self.config.command_prefix):
            handler = getattr(self, 'my_command', None)
            #return /This was originally supposed to get rid of the message if it was not an existing command, but revised it to only  filter incorrect commands out /Clicked
    ...
    async def cmd_jh(self, message): #This is a method that basically tests certain fundamental methods that I've been trying to make work
        string1 = r'\xiaoya_by_mikegybson.jpg'
        await self.send_message(message.channel, 'Current path: %s' % string1)
        await self.send_file(message.channel, PHOTO_CACHE_PATH + '%s' % string1)
        await self.send_file(message.author, r'C:\Users\Brandon\Documents\MusicBot-master\MusicBot-master\photos\xiaoya_by_mikegybson.jpg')
        this_server = discord.utils.get(self.servers, name='Every Single Person') #This is the way I can access channels, members, etc. from the server
        if message.channel == discord.utils.get(this_server.channels, name='test_channel'):
            await self.send_message(message.channel, 'This is the test channel, BY NAME')
        if message.channel == this_server.get_channel('265302474703896576'):
            await self.send_message(message.channel, 'This is the test channel, BY ID')

    async def my_command(self, message): #Manges messages that aren't handled by the command framework
        this_server = discord.utils.get(self.servers, name='Every Single Person')
        if message.author == this_server.get_member('195174230281814016') and ('...' in message.content or ',,,' in message.content): #If Amrit posts a message with "..." the bot posts a meme
            meme_num = int((random.random()*15) + 1)
            string1 = r'\amritmeme0%s.jpg' % meme_num
            await self.send_file(message.channel, AMRIT_CACHE_PATH + '%s' % string1)
        

    #My code ends here /Clicked
    #No empty lines at the end of original code /Clicked
